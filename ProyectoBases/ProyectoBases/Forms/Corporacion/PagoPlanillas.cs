...
namespace ProyectoBases
{
    public partial class PagoPlanillas : Form
    {
        ...
        public void cargarPlanillas() // carga la planilla a la tabla para proceder a pagar, estas son planillas de cualquier planta pero deberia ser para cada una
        {
            string query = "EXEC verPlanillas";
        ...
        private void pagarBTN_Click(object sender, EventArgs e)
        {
            if (int.TryParse(idPlanillaText.Text, out int idPlanilla))
            {
               ...
               // MessageBox.Show("PASO POR PAGAR PLANILLA.", "BIEN", MessageBoxButtons.OK, MessageBoxIcon.Error);
                this.Close();
            }
            else
            {
